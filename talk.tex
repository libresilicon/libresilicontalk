...
\begin{frame}{Definition of the Routing Hierarchy}
	\begin{itemize}
        \setlength\itemsep{2em}
            \item Academia + Industry:
	    \begin{itemize}
            \setlength\itemsep{1em}
		\item Geographical partitioning of a wafer: cut tree
		\item Based on preceeding floorplanning steps
	    \end{itemize}
            \item LibreSilicon:
	    \begin{itemize}
            \setlength\itemsep{1em}
		\item Modular chip development: subcell hierarchy
		\item Nodes carry implicit subcells as well
	    \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{High modularization}
       \begin{figure}
        \centering
        \includegraphics[scale=0.32]{SystemBus.png}
       \end{figure}
\end{frame}
